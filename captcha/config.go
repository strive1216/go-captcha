...
func GetCaptchaDefaultConfig() *Config {
	return &Config{
		...
		rangFont:            DefaultBinFontList(),
		rangThumbFont:       DefaultBinFontList(),
		rangBackground:      DefaultBinImageList(),
		rangThumbBackground: DefaultBinThumbImageList(),
	}
}
 ...
 * @return [][]byte
 */
func DefaultBinFontList() [][]byte {
	dir, _ := _fonts.ReadDir("resources/fonts")
	var fonts [][]byte
	for _, file := range dir {
		if file.IsDir() {
			continue
		}
		_font, _ := _fonts.ReadFile("resources/fonts/" + file.Name())
		fonts = append(fonts, _font)
	}
	return fonts
}
 ...
 * @return [][]byte
 */
func DefaultBinImageList() [][]byte {
	dir, _ := _imgs.ReadDir("resources/images")
	var fonts [][]byte
	for _, file := range dir {
		if file.IsDir() {
			continue
		}
		_font, _ := _imgs.ReadFile("resources/images/" + file.Name())
		fonts = append(fonts, _font)
	}
	return fonts
}

//go:embed resources/images/thumb/*.jpg
var _imgsThumb embed.FS

// DefaultBinThumbImageList
/**
 * @Description: 内置默认资源
 * @return [][]byte
 */
func DefaultBinThumbImageList() [][]byte {
	dir, _ := _imgsThumb.ReadDir("resources/images/thumb")
	var fonts [][]byte
	for _, file := range dir {
		if file.IsDir() {
			continue
		}
		_font, _ := _imgsThumb.ReadFile("resources/images/thumb/" + file.Name())
		fonts = append(fonts, _font)
	}
	return fonts
}
